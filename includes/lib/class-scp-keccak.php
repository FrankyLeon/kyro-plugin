<?php
/**
 * Keccak-256 (Ethereum), adapted from kornrunner/php-keccak (MIT).
 */
if ( ! defined( 'ABSPATH' ) ) {
    exit;
}

final class SCP_Keccak {
    ...
    public static function hash256( $in, $raw_output = true ) {
        return self::hash( $in, 256, $raw_output );
    }

    public static function hash( $in, $mdlen = 256, $raw_output = false ) {
        return ( PHP_INT_SIZE === 8 )
            ? self::keccak64( $in, $mdlen, $mdlen, self::LFSR, $raw_output )
            : self::keccak32( $in, $mdlen, $mdlen, self::LFSR, $raw_output );
    }

    private static function strlen8( $s ) {
        return function_exists( 'mb_strlen' ) ? mb_strlen( $s, self::ENCODING ) : strlen( $s );
    }

    private static function substr8( $s, $start, $length = null ) {
        if ( function_exists( 'mb_substr' ) ) {
            return $length === null
                ? mb_substr( $s, $start, null, self::ENCODING )
                : mb_substr( $s, $start, $length, self::ENCODING );
        }
    ...
    private static function keccak64( $in_raw, $capacity, $outputlength, $suffix, $raw_output ) {
        ...
        for ( $in_t = 0; $inlen >= $rsiz; $inlen -= $rsiz, $in_t += $rsiz ) {
            for ( $i = 0; $i < $rsizw; $i++ ) {
                $t      = unpack( 'V*', self::substr8( $in_raw, (int) ( $i * 8 + $in_t ), 8 ) );
                $st[ $i ] = [
                    $st[ $i ][0] ^ $t[2],
                    $st[ $i ][1] ^ $t[1],
                ];
        ...
        $temp = self::substr8( $in_raw, (int) $in_t, (int) $inlen );
        ...
        for ( $i = 0; $i < $rsizw; $i++ ) {
            $t        = unpack( 'V*', self::substr8( $temp, $i * 8, 8 ) );
            $st[ $i ] = [
                $st[ $i ][0] ^ $t[2],
                $st[ $i ][1] ^ $t[1],
            ];
        ...
        $r = self::substr8( $out, 0, (int) ( $outputlength / 8 ) );
